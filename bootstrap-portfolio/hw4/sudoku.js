...
var printBoard = function () {   
    for(var i = 0; i<sol[0].length; i++){
        for(var j = 0; j<sol.length; j++){
            var str = "r" + (i+1) + (j+1);
            document.getElementById(str).textContent = sol[i][j];
            console.log(str);
        }
    }
};
...
var loadNewPuzzle = function() {
    ...
    //use this end point to get a new sudoku board
     request.open("GET", "https://sudoku-api.vercel.app/api/dosuku");
    
     request.onload = function(){
        var data = JSON.parse(this.response);
        console.log(data);
        //drilling down to the new grid and assigning the value to our board
